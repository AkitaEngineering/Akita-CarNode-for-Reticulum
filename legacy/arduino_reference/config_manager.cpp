...
static void loadStringPreference(const char* key, char* destination, size_t destinationSize, const char* defaultValue) {
    if (destination == nullptr || destinationSize == 0) {
        return;
    }

    String value = preferences.getString(key, String(defaultValue != nullptr ? defaultValue : ""));
...
bool loadConfig() {
    ...
    loadStringPreference("vehicle_id", runtimeConfig.vehicle_id, sizeof(runtimeConfig.vehicle_id), VEHICLE_ID);

    loadStringPreference("obdii_name", runtimeConfig.obdii_device_name, sizeof(runtimeConfig.obdii_device_name), OBDII_DEVICE_NAME);
    loadStringPreference("obdii_svc", runtimeConfig.obdii_service_uuid, sizeof(runtimeConfig.obdii_service_uuid), "");
    loadStringPreference("obdii_char", runtimeConfig.obdii_characteristic_uuid, sizeof(runtimeConfig.obdii_characteristic_uuid), "");
    ...
    loadStringPreference("wifi_ssid", runtimeConfig.wifi_ssid, sizeof(runtimeConfig.wifi_ssid), WIFI_SSID);
    loadStringPreference("wifi_pass", runtimeConfig.wifi_password, sizeof(runtimeConfig.wifi_password), WIFI_PASSWORD);

    loadStringPreference("rns_app", runtimeConfig.reticulum_app_name, sizeof(runtimeConfig.reticulum_app_name), RETICULUM_APP_NAME);
    loadStringPreference("rns_dest", runtimeConfig.reticulum_destination_address, sizeof(runtimeConfig.reticulum_destination_address), RETICULUM_DESTINATION_ADDRESS);
...
const char* getOBDIIServiceUUID() {
    return runtimeConfig.obdii_service_uuid;
}

const char* getOBDIICharacteristicUUID() {
    return runtimeConfig.obdii_characteristic_uuid;
}
...
bool getUseOBDIIUUIDs() {
    return runtimeConfig.use_obdii_uuids;
}

bool getUseOBDIINordicUART() {
    return runtimeConfig.use_obdii_nordic_uart;
}
