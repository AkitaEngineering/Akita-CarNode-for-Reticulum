...
void initReticulum() {
    ...
    const bool useWiFi = getUseWiFiForReticulum();
...
#elif !USE_LORA_FOR_RETICULUM
    if (useWiFi) {
        DEBUG_PRINTLN(F("[RNS_INIT_ERROR] Runtime config requests WiFi, but WiFi Reticulum support is not compiled in."));
    }
#endif

#if USE_LORA_FOR_RETICULUM
    if (!useWiFi) {
...
#elif !USE_WIFI_FOR_RETICULUM
    if (!useWiFi) {
        DEBUG_PRINTLN(F("[RNS_INIT_ERROR] Runtime config requests LoRa, but LoRa Reticulum support is not compiled in."));
    }
#endif
