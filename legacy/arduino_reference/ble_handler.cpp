...
static const unsigned long OBD_RESPONSE_BUFFER_TIMEOUT_MS = 2000;

static bool useConfiguredUUIDs() {
    if (getUseOBDIIUUIDs() && strlen(getOBDIIServiceUUID()) > 0 && strlen(getOBDIICharacteristicUUID()) > 0) {
        return true;
    }
...
static const char* getConfiguredServiceUUID() {
    if (strlen(getOBDIIServiceUUID()) > 0) {
        return getOBDIIServiceUUID();
    }

#ifdef USE_OBDII_UUIDS
    return OBDII_SERVICE_UUID;
#else
    return "";
#endif
}

static const char* getConfiguredCharacteristicUUID() {
    if (strlen(getOBDIICharacteristicUUID()) > 0) {
        return getOBDIICharacteristicUUID();
    }

#ifdef USE_OBDII_UUIDS
    return OBDII_CHARACTERISTIC_UUID;
#else
    return "";
#endif
}

static bool useConfiguredNordicUART() {
    if (getUseOBDIINordicUART()) {
        return true;
    }

#ifdef USE_OBDII_NORDIC_UART_SERVICE
    return true;
#else
    return false;
#endif
...
static const char* getConfiguredNUSServiceUUID() {
#ifdef USE_OBDII_NORDIC_UART_SERVICE
    return NUS_SERVICE_UUID_STR;
#else
    return "";
#endif
}

static const char* getConfiguredNUSCharacteristicTXUUID() {
#ifdef USE_OBDII_NORDIC_UART_SERVICE
    return NUS_CHARACTERISTIC_TX_UUID_STR;
#else
    return "";
#endif
}

static const char* getConfiguredOBDDeviceName() {
    const char* runtimeName = getOBDIIDeviceName();
    if (runtimeName != nullptr && strlen(runtimeName) > 0) {
        return runtimeName;
    }

    return OBDII_DEVICE_NAME;
}
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice) {
        ...
        if (useConfiguredUUIDs()) {
        // If using UUIDs, check if device has the service UUID
            if (advertisedDevice.haveServiceUUID()) {
                BLEUUID serviceUUID = advertisedDevice.getServiceUUID();
                if (serviceUUID.toString() == String(getConfiguredServiceUUID())) {
                    matches = true;
                    targetServiceUUID = String(getConfiguredServiceUUID());
                    targetCharacteristicUUID = String(getConfiguredCharacteristicUUID());
                }
            }
        }
        else if (useConfiguredNordicUART()) {
        // Check for Nordic UART Service
            if (advertisedDevice.haveServiceUUID()) {
                BLEUUID serviceUUID = advertisedDevice.getServiceUUID();
                if (serviceUUID.toString() == String(getConfiguredNUSServiceUUID())) {
                    matches = true;
                    targetServiceUUID = String(getConfiguredNUSServiceUUID());
                    targetCharacteristicUUID = String(getConfiguredNUSCharacteristicTXUUID());
                }
            }
        }
        else {
        // Connect by device name
            if (advertisedDevice.getName() == String(getConfiguredOBDDeviceName())) {
                matches = true;
            }
        }
...
static void notifyCallback(BLERemoteCharacteristic* pBLERemoteCharacteristic,
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        if (c == '\r' || c == '\n') {
            ...
            if (obdResponseBuffer.length() > 0 && !obdResponseBuffer.endsWith("\n")) {
                obdResponseBuffer += '\n';
...
void requestOBDPID(const OBD_PID& pid) {
    ...
    String request = String(pid.code) + "\r";
...
void processOBDResponse(float* rpm, float* speed_kmh, float* coolant_temp_c) {
    ...
    if (obdResponseBuffer.length() > 0 && 
        (millis() - lastOBDResponseTime > OBD_RESPONSE_BUFFER_TIMEOUT_MS)) {
